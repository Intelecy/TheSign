...
def neighbors(i: int) -> list[int]:
    if i >= 0 and i < len(neighbors_list):
        return neighbors_list[i]
    else:
        raise Exception("Uknown index")
...
def make_snake(length: int):
    ...
    while len(snake) < length:
        ...
        # if we fail to find any free neightbors, just return
        if len(f) == 0:
            return snake

        next = random.choice(f)
        snake.append(next)
    return snake
...
class State:
    ...
    def update(self):
        ...
        if len(f) == 0:
            # oh no! we ran into ourself. For now just reset
            self.reset()
            return
...
class Snake(Animation):
    def __init__(self, *args, **kwargs):
        ...
        self.frames_per_update = self.frame_rate // 2
